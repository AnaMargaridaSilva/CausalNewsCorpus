...
def main():
    ...
    # Optionally send telemetry if desired
    # send_example_telemetry("run_ner_no_trainer", args)

    # Initialize the accelerator
    accelerator = Accelerator()   # No with_tracking argument
    ...
    # Initialize W&B only if tracking is enabled
    # if args.with_tracking:
    #    try:
    #        import wandb  # Make sure W&B is imported here
    #        wandb.init(config=args)  # Initialize W&B with config if args contains model configuration
    #        wandb.run.log_code(".")
    #    except Exception as e:
    #        logger.warning(f"W&B initialization failed: {e}")

    # Set logging verbosity based on process
    ...
    if args.do_test:
        ...
        if args.do_train:  # continuing from above
            if args.use_best_model:  # either use best model, or use last model
                load_checkpoint_for_test = os.path.join(args.output_dir, f"epoch_{best_epoch}/model.safetensors")
                accelerator.print(f"Load best model: {load_checkpoint_for_test}")
                model.load_state_dict(torch.load(load_checkpoint_for_test, weights_only=True))
            else:
            ...
            model.load_state_dict(torch.load(args.load_checkpoint_for_test, weights_only=True))
        ### ATÉ AQUI
